...
int main() {
    ...
    const int S=0,T=500,FD=222,DR=333;
    for (int i=1; i<=F; i++) d.AddEdge(S,FD+i,1);
    for (int i=1; i<=D; i++) d.AddEdge(DR+i,T,1);
    for (int i=1; i<=N; i++) d.AddEdge(i<<1,i<<1|1,1);

    for (int i=1; i<=N; i++) {
        int fi,di; cin>>fi>>di;
        for (int j=0; j<fi; j++) {
            int x; cin>>x;
            d.AddEdge(FD+x,i<<1,1);
        }
        for (int j=0; j<di; j++) {
            int x; cin>>x;
            d.AddEdge(i<<1|1,DR+x,1);
        }
    }
